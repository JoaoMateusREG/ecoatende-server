...
export class UpdateCardUseCase {
  ...
  async execute(updateCardDto: UpdateCardDto & { id: number }): Promise<Card> {
    ...
    if (
      updateCardDto.datehourAttend &&
      typeof updateCardDto.datehourAttend === 'string'
    ) {
      processedDto.datehourAttend = new Date(updateCardDto.datehourAttend);
    }
    if (
      updateCardDto.datehourConcluded &&
      typeof updateCardDto.datehourConcluded === 'string'
    ) {
      processedDto.datehourConcluded = new Date(
        updateCardDto.datehourConcluded,
      );
    }
