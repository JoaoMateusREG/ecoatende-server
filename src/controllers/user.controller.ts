...
export class UserController {
  ...
  @ApiResponse({ status: 400, description: 'Dados inválidos' })
  async changePassword(@Body() changePasswordDto: ChangePasswordDto, @CurrentUser() currentUser: CurrentUserType) {
    try {
  ...
  @ApiParam({ name: 'cpf', description: 'CPF do usuário', example: '123.456.789-01' })
  @ApiResponse({ status: 200, description: 'Usuário atualizado com sucesso' })
  @ApiResponse({ status: 404, description: 'Usuário não encontrado' })
  async update(@Param('cpf') cpf: string, @Body() updateUserDto: UpdateUserDto) {
    try {
  ...
  async findByOrganization(@Param('organizationCnpj') organizationCnpj: string) {
    try {
      const users = await this.findUsersByOrganizationUseCase.execute(organizationCnpj);
      return users;
    } catch (error: any) {
      throw new HttpException(
        { error: error.message },
